...
{
	if (m_InputMgr->isKeyDown(VK_RIGHT)&& spritePos2D.x<=768-textureWidth/2)
	{
		spritePos2D.x += 5.0f;
	}
	if (m_InputMgr->isKeyDown(VK_LEFT) && spritePos2D.x >= textureWidth/2)
	{
		spritePos2D.x -= 5.0f;
	}
	if (m_InputMgr->isKeyDown(VK_UP) && spritePos2D.y >= 500 - textureHeight)
	{
		spritePos2D.y -= 5.0f;
	}
	if (m_InputMgr->isKeyDown(VK_DOWN) && spritePos2D.y >= 1024 - textureHeight)
	{
		spritePos2D.y += 5.0f;
	}
